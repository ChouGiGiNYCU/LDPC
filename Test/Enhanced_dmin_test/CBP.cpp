...
int main(int argc,char* argv[]){
    ...
    Read_File_H(&H,PCM_file_name);

    // define Origin H
    struct parity_check Origin_H;
    Read_File_H(&Origin_H,Origin_PCM_file);
    ...
    double SNR = SNR_min;
    vector<int> transmit_codeword(Origin_H.n,0);
    vector<double> receiver_LLR(H.n,0);
    double code_rate = (double)(Origin_H.n-Origin_H.m)/(double)Origin_H.n;
    ...
    vector<double> guess(Origin_H.n,0);
    ...
    while(SNR <= SNR_max){
        ...
        while(frame_error < frame_error_lowwer_bound){
            ...
            // assign LLR
            for(int i=0;i<Origin_H.n;i++){
                int first_pos = Origin_H.n + i;
                int second_pos = Origin_H.n*3+i;
                receiver_LLR[first_pos] = receiver_LLR[i];
                receiver_LLR[second_pos] = receiver_LLR[i];
                receiver_LLR[i] = 0;
            }
            it=0;
            ...
            while(it<iteration_limit && error_syndrome){
                ...
                for(int CN=0;CN<Origin_H.m;CN++){
                    int count=0;
                    for(int i=0;i<Origin_H.max_row_arr[CN];i++){
                        int VN = H.VN_2_CN_pos[CN][i];
                        count+=guess[VN];
                    }
                    if(count%2==1){
            ...
            // 如果syndrome是對的 但是 CodeWord 跟傳送的不一樣的話，ber 也要跟著增加
            if(!error_syndrome && bit_error_flag){
                // cout << "Guess wrong Codeword !!\n";
