...
int main(int argc,char* argv[]){
    ...
    string Superpostion_Payload_Extra_NewStructure_pos_file = argv[6]; // 新的結構 一個被punc過的Extra 會跟 兩個payload bits做結合
    cout << "##############################" << "\n";
    ...
    for(long long payload_num=1;payload_num<(1LL)<<payload_bits_num;payload_num++){
        ...
        for(long long extra_num=0;extra_num<(1LL)<<extra_bits_num;extra_num++){
            ...
            for(auto& table:Superposition_origin){
                ...
                Xor_CodeWord[payload_pos] = payload_Encode[payload_pos] ^ extra_Encode[extra_pos];
            }
            for(auto& table:Superpostion_Payload_Extra_NewStructure){
                ...
                Xor_CodeWord[payload_vn_punc] = payload_Encode[payload_vn_punc] ^ payload_Encode[payload_vn_nonpunc] ^ extra_Encode[extra_vn];
            }
