...
int main(int argc,char* argv[]){
    ...
    string PayLoad_G_file = argv[3]==string("None")?"":argv[3];
    string PayLoad_out_file = argv[4];
    
    string puncture_pos_file = argv[5];
    ...
    vector<int> punc_map;
    punc_map = Read_punc_pos(puncture_pos_file,punc_nums);
    ...
    while(SNR <= SNR_max){
        ...
        while(payload_frame_error < frame_error_lowwer_bound){
            ...
            for(int i=PayLoad_H.n;i<H.n;i++){
                int punc_pos = punc_map[i];
                receiver_LLR[i] = receiver_LLR[punc_pos]; 
                receiver_LLR[punc_pos] = 0;
            }
