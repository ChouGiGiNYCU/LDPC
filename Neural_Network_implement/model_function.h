...
public:
    ...
    VNUpdateLayer(const int input_size,const int output_size,string fc_name,const string& weight_file,const string& bias_file) {
        this->input_size = input_size;
        this->output_size = output_size;
        this->fc_name = fc_name;
        Read_Weight_File(weight_file,bias_file);
    ...
    // 計算output
    const vector<float>& forward(const vector<float>& input,const vector<float> & channel_LLR) {
        if (input.size() != this->input_size) {
            cout << this->fc_name <<" - Input size mismatch. Expected " << this->input_size << " but got " << input.size() << endl;
            exit(1);  // 可根據需求處理錯誤
        }
        ...
        for(int c=0;c<this->output_size;c++){
            ...
            this->neural_output[c] += bias_input[c]*channel_LLR[c];
        } 
        return this->neural_output;
    }
