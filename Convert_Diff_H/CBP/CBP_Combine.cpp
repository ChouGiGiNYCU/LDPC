...
int main(int argc,char* argv[]){
    ...
    string file_name_out = argv[3];
    string G_file_path  = argv[4];
    bool Encode_Flag = atoi(argv[5])==1?true:false;
    int iteration_limit = atoi(argv[6]);
    double SNR_min = atof(argv[7]);
    double SNR_max = atof(argv[8]);
    double SNR_ratio = atof(argv[9]);
    int Each_it1 = atoi(argv[10]);
    int Each_it2 = atoi(argv[11]);
    cout << "##############################" << endl;
    cout << "file_name : " << filename1 << endl;
    cout << "file_name : " << filename2 << endl;
    ...
    cout << "Each_it1 : " << Each_it1 << endl;
    cout << "Each_it2 : " << Each_it2 << endl;
    cout << "##############################" << endl;
    // define H
    struct parity_check H1,H2;
    Read_File_H(&H1,filename1);
    Read_File_H(&H2,filename2);
    // define G
    ...
    double SNR = SNR_min;
    double *transmit_codeword = (double*)calloc(H1.n,sizeof(double));
    double *receiver_LLR = (double*)calloc(H1.n,sizeof(double));

    double code_rate = (double)(H1.n-H1.m)/(double)H1.n;
    double ** CN_2_VN_LLR = (double**)malloc(sizeof(double*)*H1.n);
    for(int i=0;i<H1.n;i++) CN_2_VN_LLR[i]=(double*)calloc(H1.max_col_arr[i],sizeof(double));
    double ** VN_2_CN_LLR = (double**)malloc(sizeof(double*)*H1.m);
    for(int i=0;i<H1.m;i++) VN_2_CN_LLR[i]=(double*)calloc(H1.max_row_arr[i],sizeof(double));

    double ** CN_2_VN_LLR_2 = (double**)malloc(sizeof(double*)*H2.n);
    for(int i=0;i<H2.n;i++) CN_2_VN_LLR_2[i]=(double*)calloc(H2.max_col_arr[i],sizeof(double));
    double ** VN_2_CN_LLR_2 = (double**)malloc(sizeof(double*)*H2.m);
    for(int i=0;i<H2.m;i++) VN_2_CN_LLR_2[i]=(double*)calloc(H2.max_row_arr[i],sizeof(double));

    double *guess = (double*)calloc(H1.n,sizeof(double));
    double *totalLLR1 = (double*)calloc(H1.n,sizeof(double));
    double *totalLLR2 = (double*)calloc(H2.n,sizeof(double));

    double *bit_error_count = (double*)calloc(iteration_limit,sizeof(double));
    vector<bool> information_bit((G.size()));
    vector<int> result;
    clock_t  clk_start,clk_end;
    cout << "start to do " << endl;
    while(SNR <= SNR_max){
        ...
        while(frame_error < frame_error_lowwer_bound){
            // ASSIGN encode codeword or zero codeword
            if(Encode_Flag){
                for(int i=0;i<H1.n-H1.m;i++){
            ...
            for(int i=0;i<H1.n;i++){
                ...
                // receiver_LLR[i] = 1 ; // test
            }
            for(int i=0;i<H1.n;i++){
                totalLLR2[i] = receiver_LLR[i]; 
                totalLLR1[i] = receiver_LLR[i]; 
            }
            ...
            while(it<iteration_limit && error_syndrome){
                int it1=0;
                while(it1<Each_it1 && it<iteration_limit && error_syndrome){
                    /* ------- CN update ------- */
                    for(int VN=0;VN<H1.n;VN++){
                        for(int i=0;i<H1.max_col_arr[VN];i++){
                            int CN = H1.CN_2_VN_pos[VN][i];
                            double total_LLR = 1;
                            double alpha=1,phi_beta_sum=0;
                            for(int j=0;j<H1.max_row_arr[CN];j++){
                                int other_VN = H1.VN_2_CN_pos[CN][j];
                                if(other_VN == VN) continue; 
                                if(it1==0){ //第一次迭帶就是 直接加上剛算好的receiver_LLR
                                    phi_beta_sum += phi(abs(totalLLR2[other_VN]));
                                    alpha *= totalLLR2[other_VN]>0?1:-1;
                                    // total_LLR *= tanh(0.5*(totalLLR2[other_VN]));
                                }else{
                    ...
                    /* ------- VN update ------- */
                    for(int CN=0;CN<H1.m;CN++){
                        for(int i=0;i<H1.max_row_arr[CN];i++){
                            int VN=H1.VN_2_CN_pos[CN][i];
                            double total_LLR = totalLLR2[VN];
                            for(int j=0;j<H1.max_col_arr[VN];j++){
                                int other_CN = H1.CN_2_VN_pos[VN][j];
                    ...
                    /* ------- total LLR ------- */
                    for(int VN=0;VN<H1.n;VN++){
                        totalLLR1[VN]=totalLLR2[VN];
                        for(int i=0;i<H1.max_col_arr[VN];i++){
                            totalLLR1[VN]+=CN_2_VN_LLR[VN][i];
                        }
                        /* ------- make decision ------- */
                        guess[VN]= (totalLLR1[VN]<0)?1:0;
                    }
                ...
                int it2=0;
                while(it2<Each_it2 && it<iteration_limit && error_syndrome){
                    /* ------- CN update ------- */
                    for(int VN=0;VN<H2.n;VN++){
                        for(int i=0;i<H2.max_col_arr[VN];i++){
                            int CN = H2.CN_2_VN_pos[VN][i];
                            double alpha=1,phi_beta_sum=0;
                            double total_LLR = 1;
                            for(int j=0;j<H2.max_row_arr[CN];j++){
                                int other_VN = H2.VN_2_CN_pos[CN][j];
                                if(other_VN == VN) continue; 
                                if(it2==0){ //第一次迭帶就是 直接加上剛算好的receiver_LLR
                                    phi_beta_sum += phi(abs(totalLLR1[other_VN]));
                                    alpha *= totalLLR1[other_VN]>0?1:-1;
                                    // total_LLR *= tanh(0.5*(totalLLR1[other_VN])); 
                                }else{
                                    phi_beta_sum += phi(abs(VN_2_CN_LLR_2[CN][j]));
                                    alpha *= VN_2_CN_LLR_2[CN][j]>0?1:-1;
                                    // total_LLR *= tanh(0.5*VN_2_CN_LLR_2[CN][j]);
                                }
                            }
                            // total_LLR = 2*atanh(total_LLR);
                            // CN_2_VN_LLR_2[VN][i] = total_LLR;
                            CN_2_VN_LLR_2[VN][i]= alpha * phi(phi_beta_sum);
                        }
                    }
                    /* ------- VN update ------- */
                    for(int CN=0;CN<H2.m;CN++){
                        for(int i=0;i<H2.max_row_arr[CN];i++){
                            int VN=H2.VN_2_CN_pos[CN][i];
                            double total_LLR = totalLLR1[VN];
                            for(int j=0;j<H2.max_col_arr[VN];j++){
                                int other_CN = H2.CN_2_VN_pos[VN][j];
                                if(other_CN == CN) continue;
                                total_LLR += CN_2_VN_LLR_2[VN][j];
                            }
                            VN_2_CN_LLR_2[CN][i] = total_LLR;
                        }
                    }
                    /* ------- total LLR ------- */
                    
                    for(int VN=0;VN<H2.n;VN++){
                        totalLLR2[VN]=totalLLR1[VN];
                        for(int i=0;i<H2.max_col_arr[VN];i++){
                            totalLLR2[VN]+=CN_2_VN_LLR_2[VN][i];
                        }
                        /* ------- make decision ------- */
                        guess[VN]= (totalLLR2[VN]<0)?1:0;
                    }
                    /* Determine if it is a syndrome -> guess * H^T = vector(0) */
                    /*
                    error_syndrome = false;
                   ...
                   /* ----- Determine bit error ----- */
                    ...
                    bit_error_flag=false;
                    for(int VN=0;VN<H2.n;VN++){
                        // if(it==iteration_limit-1)printf("%.1f ",guess[VN]);
                        if(guess[VN]!=transmit_codeword[VN]){
                            bit_error_count[it]++;
                            bit_error_flag=true;
                            error_syndrome = true;
                    ...
                    // if(it==iteration_limit-1)printf("\n");
                    it++;
                    it2++;
                }
        ...
        printf("SNR : %.2f  | FER : %.5e  | BER : %.5e(%d) | Time : %.3f | Total frame : %.0f\n",SNR,frame_error/frame_count,bit_error_count[it-1]/((double)H1.n*frame_count),it,clk_duration,frame_count);
        outFile << SNR << ", " << frame_error/frame_count << ", ";
        for(int i=0;i<iteration_limit;i++){
            outFile << bit_error_count[i]/((double)H1.n*frame_count) << ", ";
        }
        outFile <<endl;
    ...
    free(receiver_LLR);
    for(int i=0;i<H1.n;i++) free(CN_2_VN_LLR[i]);
    free(CN_2_VN_LLR);
    for(int i=0;i<H1.m;i++) free(VN_2_CN_LLR[i]);
    free(VN_2_CN_LLR);
    free(guess);
