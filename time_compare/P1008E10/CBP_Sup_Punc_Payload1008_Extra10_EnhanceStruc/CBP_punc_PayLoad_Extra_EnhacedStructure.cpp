...
int main(int argc,char* argv[]){
    ...
    cout << "time_outfile : " << time_outfile << endl; 
    cout << "##############################" << "\n";
    // define H_combine
    ...
    // create time out csv
    ofstream time_out(time_outfile);
    if (!time_out.is_open()) {
    ...
    while(SNR <= SNR_max){
        ...
        while(frame < try_correct_frame ){
            ...
            for(int i=(PayLoad_H.n+Extra_H.n*2+1);i<(PayLoad_H.n+Extra_H.n*3);i++) receiver_LLR[i] = 0; // be punctured
            // setting puncture bits - Extra^Payload1^Payload2_VNs
            auto pair_iter1  = Superpostion_Payload_Extra_NewStructure.begin();
            for(int i=(PayLoad_H.n+Extra_H.n*3+1);i<(PayLoad_H.n+Extra_H.n*4) && pair_iter1!=Superpostion_Payload_Extra_NewStructure.end();i++,pair_iter1++){
