...
int main(int argc,char* argv[]){
    ...
    while(SNR <= SNR_max){
        ...
        while(payload_frame_error < frame_error_lowwer_bound){
            ...
            // PayLoad xor Extra
            for(int i=0;i<PayLoad_H.n;i++){
                transmit_codeword[i] = (payload_Encode[i] + extra_Encode[i])%2;    
            }
            ...
            // Find Min Weight
            vector<int> Guess_Extra_Encode = MinHammingWeight(Extra_G,PayLoad_H,HardDecision_result,info_bits);
            
            // Remove the interference
            for(int i=0;i<Guess_Extra_Encode.size();i++){
                receiver_LLR[i] = pow(-1,Guess_Extra_Encode[i]) * receiver_LLR[i];
            }
