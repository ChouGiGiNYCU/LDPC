...
vector<int> MinHammingWeight(vector<vector<bool>>& G,const struct parity_check& PayLoad_H, vector<int>& HardDecision_result,const int& extra_info_bits,const int& InterLeaver_col){
    ...
    for(int k=0;k<AllPossible;k++){
        ...
        vector<int> Interleaver_result = interleaver(Extra_Encode,InterLeaver_col); // Do InterLeaver
        
        // 還原可能的 PayLoad CodeWord
        for(int i=0;i<Extra_Encode.size();i++){
            PayLoad_Encode[i] = (HardDecision_result[i] + Interleaver_result[i])%2;
        }
        // PayLoad_Encode * H
