...
int main(int argc,char* argv[]){
    ...
    // Extra 使用 Payload的G來做!
    vector<bool> payload_info(PayLoad_G.size(),0);
    vector<bool> extra_info(Extra_G.size(),0);
    ...
    while(SNR <= SNR_max){
        ...
        while(payload_frame_error < frame_error_lowwer_bound){
            ...
            if(Extra_Flag){
                ...
                // 其餘 info_bits 為 0
                for(int i=info_bits;i<Extra_G.size();i++) extra_info[i] = 0;
                
                extra_Encode = Vector_Dot_Matrix_Int(extra_info,Extra_G);
                if(extra_Encode.size()!=Extra_H.n) throw invalid_argument("-- Error : Extra_Encode size  must be equal to number of colsize on Extra_H.\n");
            ...
            // Find Min Weight
            vector<int> Guess_Extra_Encode_Interleaver = MinHammingWeight(Extra_G,PayLoad_H,HardDecision_result,info_bits,InterLeaver_col);
            
            // Remove the interference
            for(int i=0;i<Guess_Extra_Encode_Interleaver.size();i++){
                receiver_LLR[i] = pow(-1,Guess_Extra_Encode_Interleaver[i]) * receiver_LLR[i];
            }
