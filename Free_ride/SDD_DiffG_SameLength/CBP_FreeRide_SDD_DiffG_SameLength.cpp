...
int main(int argc,char* argv[]){
    ...
    while(SNR <= SNR_max){
        ...
        while(payload_frame_error < frame_error_lowwer_bound){
            ...
            // Do Soft-Decision
            vector<int> Guess_Extra_Encode = FindLargeLLR(Extra_G,PayLoad_H,receiver_LLR,info_bits);
            // Remove the interference
            for(int VN=0;VN<Guess_Extra_Encode.size();VN++){
                receiver_LLR[VN] = pow(-1,Guess_Extra_Encode[VN]) * receiver_LLR[VN];
            }
