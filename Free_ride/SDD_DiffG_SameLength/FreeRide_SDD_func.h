...
vector<int> FindLargeLLR(vector<vector<bool>>& G,const struct parity_check& PayLoad_H,const vector<double>& receive_LLR,const int& info_length){
    ...
    for(int k=0;k<AllPossible;k++){
        ...
        for(int VN=0;VN<receive_LLR_copy.size();VN++){
            receive_LLR_copy[VN] = pow(-1,Extra_Encode[VN]) * receive_LLR[VN];
        } 
