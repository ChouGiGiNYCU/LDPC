...
file_name = "H_96_48.txt" # write parity_check_matrix to the txt file 
train_data_file_name = r"D:\LDPC\H_96_48\Train_weight_andbias\H_96_48_TrainLLR.txt"
train_data_target_file_name = r"D:\LDPC\H_96_48\Train_weight_andbias\H_96_48_TrainCodeWord.txt"
valid_data_file_name = r"D:\LDPC\H_96_48\Train_weight_andbias\H_96_48_ValidLLR.txt"
valid_data_target_file_name = r"D:\LDPC\H_96_48\Train_weight_andbias\H_96_48_ValidCodeWord.txt"
Encode_Train_Flag = True
Save_model_file_name = "BPNN3_Mulitiloss_H9648_"
...
if Encode_Train_Flag==True:
    train_data_target = read_data(train_data_target_file_name)
    valid_data_target = read_data(valid_data_target_file_name)
##################### Construct CN2VN #####################
CN2VN_pos = []
max_row_degree =0 
...
if Encode_Train_Flag==False:
    ...
    train_targets = torch.tensor(train_data_target,dtype=torch.float32)
    valid_targets = torch.tensor(valid_data_target,dtype=torch.float32)
# 創建完整的數據集 data->tensor
train_data_tensor = torch.tensor(train_data, dtype=torch.float32)
# train_data_tensor = train_data.clone().detach().to(torch.float32)
valid_data_tensor = torch.tensor(valid_data, dtype=torch.float32)
# valid_data_tensor = valid_data.clone().detach().to(torch.float32)

# train_targets_tensor = torch.tensor(train_targets, dtype=torch.float32)
train_targets_tensor = train_targets.clone().detach().to(torch.float32)
# valid_targets_tensor = torch.tensor(valid_targets, dtype=torch.float32)
valid_targets_tensor = valid_targets.clone().detach().to(torch.float32)
...
for epoch in range(epochs):
    ...
    for inputs, labels in train_progress:
        ...
        # print(model.bias_scaling_vectors[0])
