...
int main(int argc,char* argv[]){
    ...
    vector<bool> information_bit((H.n-H.m));
    vector<int> result;
    ...
    while(SNR <= SNR_max){
        ...
        while(frame_error < frame_error_lowwer_bound){
            ...
            for(int i=0;i<H.n;i++){
                ...
                receiver_codeword = receiver_codeword>0?1:-1; // HardDecision
                receiver_codeword*=1; // 
                receiver_LLR[i] = receiver_codeword;  // HardDecision
                // receiver_LLR[i]=2*receiver_codeword/pow(sigma,2); // HardDecision -> Sigma
            }
            it=0;
