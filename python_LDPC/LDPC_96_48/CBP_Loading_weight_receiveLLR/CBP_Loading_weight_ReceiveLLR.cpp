    ...
    while(SNR <= SNR_max){
        ...
        while(frame_error < frame_error_lowwer_bound){
            ...
            for(int i=0;i<H.n;i++){
                ...
                receiver_LLR[i] = receiver_codeword; //  
                // receiver_codeword = receiver_codeword>0?1:-1; // Hard Decision
                // receiver_LLR[i]=2*receiver_codeword/pow(sigma,2);  // Hard Decision -> Sigma
            }
            it=0;
