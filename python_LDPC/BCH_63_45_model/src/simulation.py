...
train_data_file_name = "train_data_set_63_45.txt"
valid_data_file_name = "valid_data_set_63_45.txt"
Save_model_file_name = "sparse_bp_neural_network_mulitiloss_"
Load_model_file_name = 'sparse_bp_neural_network_mulitiloss_4.pth'
CSV_file_name = "BP_model_test2.csv"
Load_setting = True
Train = False
...
if Load_setting==True:
    model.load_state_dict(torch.load(Load_model_file_name,map_location=torch.device('cpu')))
...
########################### Model Setting ###########################
    ...
    transmit_codeword = [0]*parity_check_matrix_colsize
    receiver_LLR = []
    BER = []
    FER = []
    for SNR in np.arange(SNR_MIN,SNR_MAX+1,SNR_RATIO):
        ...
        while FRAME_ERROR_COUNT < Frame_Error_Bound:
            ...
            for i in range(0,parity_check_matrix_colsize):
                receiver_codeword = -2*transmit_codeword[i] + 1 + sigma*generate_gaussian_noise()
                receiver_LLR.append(receiver_codeword)
            ...
            exit(1)
            # print(decode_codeword_tensor[0].squeeze())
            decode_codeword = decode_codeword_tensor[0].squeeze().float().tolist()  # 確保 decode_codeword 是整數列表
            # print("decode_codeword : {}".format(decode_codeword))
            FLAG = False 

            for code in decode_codeword :
                if code<0.5:
                    BER_COUNT+=1
                    FLAG = True    
            if FLAG == True:
