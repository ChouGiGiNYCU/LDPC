    ...
    */
    ...
    while(SNR <= SNR_max){
        ...
        while(payload_frame_error < frame_error_lowwer_bound){
            ...
            // 處理 Extra CodeWord 是zero 還是 Encode
            if(Extra_Flag){
            ...
            for(auto& table:Superposition_origin){
                ...
                transmit_codeword[payload_pos] = payload_Encode[payload_pos] ^ extra_Encode[extra_pos];
            }
            ...
            for(auto& table:Superpostion_Payload_Extra_NewStructure){
                ...
                if(prev_payload_vn_punc==-1) transmit_codeword[payload_vn_punc] = payload_Encode[payload_vn_punc] ^ payload_Encode[payload_vn_nonpunc] ^ extra_Encode[extra_vn];
                else                         transmit_codeword[payload_vn_punc] = transmit_codeword[prev_payload_vn_punc] ^ payload_Encode[payload_vn_punc] ^ payload_Encode[payload_vn_nonpunc] ^ extra_Encode[extra_vn];
                prev_payload_vn_punc = payload_vn_punc;
            }
            /*------------------------------------------*/
            // Count LLR
            ...
            for(int i=(PayLoad_H.n+Extra_H.n+nonTransmit_Extra_Vns_num);i<(PayLoad_H.n+Extra_H.n+nonTransmit_Extra_Vns_num+Extra_H.n);i++) receiver_LLR[i] = 0; // be punctured
            // setting puncture bits - Extra^Payload1^Payload2_VNs
            auto pair_iter1  = Superpostion_Payload_Extra_NewStructure.begin();
            for(int i=(PayLoad_H.n+Extra_H.n+nonTransmit_Extra_Vns_num+Extra_H.n);i<(PayLoad_H.n+Extra_H.n+nonTransmit_Extra_Vns_num+Extra_H.n*2) && pair_iter1!=Superpostion_Payload_Extra_NewStructure.end();i++,pair_iter1++){
            ...
            // ofstream file3("LLR.txt");
            // for(int i=0;i<receiver_LLR.size();i++){
            //     file3 << receiver_LLR[i] << "\n";
            // }
            // file3.close();
            // int cnt = 0;
            // for(int i=0;i<H.n;i++){
            //     if(receiver_LLR[i]!=0) cnt++;
            // }
            // if(cnt==1008) cout << "Transmit 1008\n"; 
            // else  cout << "Transmit is not 1008\n";
            // exit(1);
            /*------------------------------------------*/
