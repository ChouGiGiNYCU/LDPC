...
int main(int argc,char* argv[]){
    ...
    Superposition_origin = Read_Extra2Pyalod_CSVfile(puncture_pos_file);
    ...
    while(SNR <= SNR_max){
        ...
        while(payload_frame_error < frame_error_lowwer_bound){
            ...
            /*------------------------------------------*/
            // Count LLR
            fill(receiver_LLR.begin(),receiver_LLR.end(),0.0); // reset
            for(int i=0;i<PayLoad_H.n;i++){
            ...
            for(auto pair_iter = Superposition_origin.begin();pair_iter!=Superposition_origin.end();pair_iter++){
                int payload_pos = pair_iter->second;
                int extra_pos   = pair_iter->first;
                receiver_LLR[PayLoad_H.n+extra_pos] = receiver_LLR[payload_pos];
                receiver_LLR[payload_pos] = 0; // be punctured
            }
            ...
            while(it<iteration_limit && (payload_error_syndrome || extra_error_syndrome)){
                ...
                for(int VN=0;VN<H.n;VN++){
                    // 不把訊息傳到Extra 那邊
                    if(iteration_open_flag && VN>=PayLoad_H.n  && (it<iteration_open && payload_correct_flag==false)) continue;
                ...
                if(payload_error_syndrome==false && flip==false){
                    flip = true;
                    for(auto pair_iter = Superposition_origin.begin();pair_iter!=Superposition_origin.end();pair_iter++){
                        int payload_pos = pair_iter->second;
                        int extra_pos   = pair_iter->first;
                        receiver_LLR[PayLoad_H.n+extra_pos] = totalLLR[payload_pos]>0?receiver_LLR[PayLoad_H.n+extra_pos]:-1*receiver_LLR[PayLoad_H.n+extra_pos];
                    }
                }
                ...
                // flip superposition extra position
