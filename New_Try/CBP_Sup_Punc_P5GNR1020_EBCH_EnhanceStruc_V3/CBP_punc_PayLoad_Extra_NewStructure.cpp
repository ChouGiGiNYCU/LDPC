...
int main(int argc,char* argv[]){
    ...
    string Superpostion_Payload_Extra_NewStructure_pos_file = argv[14]; // 新的結構 一個被punc過的Extra 會跟 兩個payload bits做結合
    int Z = atoi(argv[15]);
    cout << "##############################" << "\n";
    ...
    while(SNR <= SNR_max){
        ...
        while(payload_frame_error < frame_error_lowwer_bound){
            ...
            for(auto& table:Superpostion_Payload_Extra_NewStructure){
                int extra_vn1  = table.extra_vn;
                int payload_vn_nonpunc  = table.payload_vn_nonpunc;
                int payload_vn_punc  = table.payload_vn_punc;
                transmit_codeword[payload_vn_punc] = payload_Encode[payload_vn_punc] ^ payload_Encode[payload_vn_nonpunc] ^ extra_Encode[extra_vn1];
            }
            /*------------------------------------------*/
            // Count LLR
            ...
            for(int i=(PayLoad_H.n+Extra_H.n);i<(PayLoad_H.n+Extra_H.n*2);i++) receiver_LLR[i] = 0; // be punctured
            // setting puncture bits - Extra^Payload1^Payload2_VNs
            auto pair_iter1  = Superpostion_Payload_Extra_NewStructure.begin();
            for(int i=(PayLoad_H.n+Extra_H.n*2);i<(PayLoad_H.n+Extra_H.n*3) && pair_iter1!=Superpostion_Payload_Extra_NewStructure.end();i++,pair_iter1++){
