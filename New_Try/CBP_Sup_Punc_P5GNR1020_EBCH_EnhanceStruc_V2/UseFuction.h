...
struct New_Strcuture {
    int extra_vn1;
    int extra_vn2;
    int payload_vn_nonpunc;
    int payload_vn_punc;
};
...
std::vector<New_Strcuture> Read_NewStrcuture_CSVfile(const std::string& file_name) {
    std::ifstream file(file_name);
    ...
    std::vector<New_Strcuture> records;
    while (std::getline(file, line)) {
        std::stringstream ss(line);
    ...
    for (const auto& rec : records) {
        int extra, payload, third;
        std::cout 
            << "Extra1=" << rec.extra_vn1
            << "Extra2=" << rec.extra_vn2
            << ", Non_Punc_Payload=" << rec.payload_vn_nonpunc
            << ", Punc_Payload=" << rec.payload_vn_punc 
            << "\n";
    }
    return records;
}
