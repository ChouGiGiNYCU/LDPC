...
% end
%% define 各個連接的矩陣
M_puncpos = zeros([H2_c,H1_c]);
for r=1:H2_c
    punc_vn = punc_pos_bits_origin(r);
    M_puncpos(r,punc_vn) = 1;
end
...
punc_payload_mat_NewStructure1 =  punc_payload_mat_NewStructure2;

%%
% H_combine = [  H1     zero1       zero2                     zero4   zero4 
%               zero3     H2        zero5                     zero6   zero6 
%               M_puncpos    I           I      zero7   zero7
...
T = table([1:H2_c].',punc_pos_bits_New_Struct2.',fliplr(punc_pos_bits_New_Struct2).','VariableNames', {'Extra bits','Payloadb_NonPunc(b)', 'Payloadc_Punc(c)'});  % 建立 table
