...
void BP_for_CombineH(const struct parity_check& H, const struct parity_check& PayLoad_H, const struct parity_check& Extra_H, const int& iteration_limit,const int& iteration_open,
                     const vector<double>& receiver_LLR,const vector<int>& Payload_CodeWord,bool& decode_Payload_flag,const vector<int>& Extra_CodeWord,vector<bool>& decode_Extra_flag,int Extra_PCM_number){
    ...
    vector<vector<int>> Extra_Guess_CodeWord (Extra_PCM_number,vector<int>(Extra_H.n,0));
    ...
    while(it<iteration_limit && (payload_error_syndrome || extra_error_syndrome)){
        ...
        for(int PCM=1;PCM<=Extra_PCM_number;PCM++){
            ...
            for(int VN=start_VN;VN<end_VN;VN++){
                Extra_Guess_CodeWord[PCM-1][VN-start_VN] = totalLLR[VN]>0?0:1;
            }
        }
        ...
        for(int PCM=0;PCM<Extra_PCM_number;PCM++){
            for(int CN=0;CN<Extra_H.m;CN++){
                ...
                for(int i=0;i<Extra_H.max_row_arr[CN];i++){
                    ...
                    count+=Extra_Guess_CodeWord[PCM][VN];
                }
                if(count%2==1){
    ...
    for(int PCM=0;PCM<Extra_PCM_number;PCM++){
        for(int vn=0;vn<Extra_H.n;vn++){
            if(Extra_Guess_CodeWord[PCM][vn]!=Extra_CodeWord[vn]){
                decode_Extra_flag[PCM] = false;
                break;
            }
        }
    }
