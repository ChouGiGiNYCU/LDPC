...
int main(int argc,char* argv[]){
    ...
    string Superpostion_Payload_Extra_NewStructure_pos_file = argv[15]; // 新的結構 一個被punc過的Extra 會跟 兩個payload bits做結合
    string Each_it_payload_fer_file = "Each_it_payload_fer.txt";
    cout << "##############################" << "\n";
    ...
    ofstream Each_It_Payload_Fer(Each_it_payload_fer_file);
    Each_It_Payload_Fer << "SNR" << string(5, ' ') << "iteration " << string(5, ' ') << "FER" << string(5, ' ') << "BER" << "\n";
    ...
    cout << "nonTransmit_Extra_Vns_num : " << nonTransmit_Extra_Vns_num << endl;
    while(SNR <= SNR_max){
        ...
        int it;
        vector<int> each_it_payload_fer_count(iteration_limit,0);
        ...
        while(payload_frame_error < frame_error_lowwer_bound){
            ...
            while(it<iteration_limit && (payload_error_syndrome || extra_error_syndrome)){
                ...
                if(payload_bit_error_flag==true){
                    each_it_payload_fer_count[it]++;
                    if(each_it_payload_fer_count[it]==frame_error_lowwer_bound){
                        Each_It_Payload_Fer << SNR << string(8, ' ') << it+1 << string(10, ' ') << (double)each_it_payload_fer_count[it]/frame_count << string(6, ' ') << payload_bit_error_count[it]/((double)H.n*frame_count) <<"\n";
                    }
                }
