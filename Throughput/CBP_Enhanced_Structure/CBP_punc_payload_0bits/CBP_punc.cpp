...
int main(int argc,char* argv[]){
    ...
    string puncture_pos_file = argv[9];
    int puncture_bits =  atoi(argv[10]);
    string Each_iteration_FER_file = "Each_it_FER.txt"; 
    cout << "##############################" << endl;
    ...
    cout << "puncture_bits : " << puncture_bits << endl;
    cout << "##############################" << endl;
    // define H
    ...
    // find punc
    vector<int> punc_map;
    punc_map = Read_punc_pos(puncture_pos_file,puncture_bits);
    ...
    ofstream Each_It_Fer(Each_iteration_FER_file);
    Each_It_Fer << "SNR" << string(5, ' ') << "iteration " << string(5, ' ') << "FER" << string(5, ' ') << "BER" << "\n";
    double SNR = SNR_min;
    double *transmit_codeword = (double*)calloc(H.n,sizeof(double));
    
    double *receiver_LLR = (double*)malloc(H.n*sizeof(double));
    // double code_rate = (double)(H.n-H.m)/(double)(H.n-punc_map.size()); // coderate 要改成 k/(n-punc_number bits)
    double code_rate = 1;
    ...
    while(SNR <= SNR_max){
        ...
        vector<int> each_it_fer_count(iteration_limit,0);
        clk_start = clock();
        while(frame_error < frame_error_lowwer_bound){
            ...
            while(it<iteration_limit && error_syndrome){
                ...
                if(bit_error_flag){
                    each_it_fer_count[it]++;
                    if(each_it_fer_count[it]==frame_error_lowwer_bound){
                        Each_It_Fer << SNR << string(8, ' ') << it+1 << string(10, ' ') << (double)each_it_fer_count[it]/frame_count << string(6, ' ') << bit_error_count[it]/((double)H.n*frame_count) <<"\n";
                    }
                }
                it++;
            }
        ...
        //change frame error rate 
        if(bit_error_count[iteration_limit-1]/((double)H.n*frame_count) < 1e-4){
