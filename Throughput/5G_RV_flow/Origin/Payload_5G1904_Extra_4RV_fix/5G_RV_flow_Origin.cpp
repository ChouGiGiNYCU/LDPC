...
int main(int argc,char* argv[]){
    ...
    while(SNR<=SNR_max){
        ...
        for(double frame_count=0;frame_count<total_frame_count;frame_count++){
            ...
            vector<int> Tx_CodeWord(PayLoad_H.n,0);
            ...
            vector<double> Fourth_tx_LLR(PayLoad_H.n,0);
            if(PayLoad_Flag){
                ...
                Tx_CodeWord = Vector_Dot_Matrix_Int(info_bits,PayLoad_G);
                for(int i=0;i<Tx_CodeWord.size();i++){
                    Tx_CodeWord[i] = Tx_CodeWord[i]%2;
                }
            }
            for(int i=RV0_pos*Z;i<(RV0_pos*Z+Each_ZNum*Z);i++){
                double receive_value=(double)(-2*Tx_CodeWord[i]+1) + sigma*gasdev();
            ...
            for(int i=RV2_pos*Z;i<(RV2_pos*Z+Each_ZNum*Z);i++){
                double receive_value=(double)(-2*Tx_CodeWord[i]+1) + sigma*gasdev();
                double LLR = 2*receive_value/pow(sigma,2); 
                Second_tx_LLR[i] = LLR;
                Third_tx_LLR[i] = LLR;
            ...
            for(int i=RV3_pos*Z;i<PayLoad_H.n;i++){
                double receive_value=(double)(-2*Tx_CodeWord[i]+1) + sigma*gasdev();
                double LLR = 2*receive_value/pow(sigma,2); 
                if(i<(RV2_pos*Z+Each_ZNum*Z)) Third_tx_LLR[i] = (Third_tx_LLR[i]+LLR)/2; // combine LLR
                else Third_tx_LLR[i] = LLR;
            }

            for(int i=(RV0_pos*Z);i<RV0_pos*Z+(Each_ZNum*Z-(PayLoad_H.n-RV3_pos*Z));i++){
                double receive_value=(double)(-2*Tx_CodeWord[i]+1) + sigma*gasdev();
                double LLR = 2*receive_value/pow(sigma,2); 
                Third_tx_LLR[i] = (Third_tx_LLR[i]+LLR)/2; // combine LLR
            }

            /* ------------- Process RV0 ------------- */
            bool decode_flag_A = BP_for_Payload(PayLoad_H,iteration_limit,First_tx_LLR,Tx_CodeWord);
            total_bits += Each_ZNum*Z; // total bits
            ...
            /* ------------- Process RV1 ------------- */
            bool decode_flag_B = BP_for_Payload(PayLoad_H,iteration_limit,Second_tx_LLR,Tx_CodeWord);
            total_bits += Each_ZNum*Z;
            if(decode_flag_B == true){
                correct_bits += (PayLoad_H.n - PayLoad_H.m); // info bits
                RV1++;
            }
            /* ------------- Process RV2s ------------- */
            bool decode_flag_C = BP_for_Payload(PayLoad_H,iteration_limit,Third_tx_LLR,Tx_CodeWord);
            total_bits += Each_ZNum*Z;
            if(decode_flag_C == true){
                correct_bits += (PayLoad_H.n - PayLoad_H.m); // info bits
                RV2++;
            }
        }
