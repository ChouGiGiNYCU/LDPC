...
int main(int argc,char* argv[]){
    ...
    int Z = atoi(argv[8]);
    int RV0_pos = atoi(argv[9]);
    int RV1_pos = atoi(argv[10]);
    int RV2_pos = atoi(argv[11]);
    int RV3_pos = atoi(argv[12]);
    ...
    while(SNR<=SNR_max){
        ...
        for(double frame_count=0;frame_count<total_frame_count;frame_count++){
            ...
            vector<double> Second_tx_LLR(PayLoad_H.n,0);
            vector<double> Third_tx_LLR(PayLoad_H.n,0);
            if(PayLoad_Flag) First_tx_LLR = ProcessCodeWord_ReturnLLR(PayLoad_G,PayLoad_H,PayLoad_Flag,sigma,A_CodeWord); // sigmas =1
            else{ // all zero test
                for(int i=0;i<PayLoad_H.n;i++){ // Count LLR  | BPSK -> {0->1}、{1->-1}
                    double receive_value= 1.0 + sigma*gasdev(); // fixed power , sigma=1
                    First_tx_LLR[i] = 2*receive_value/pow(sigma,2); 
                }
            }
            // 5G previous 2*Z  puncture
            for(int i=0;i<RV0_pos*Z;i++) First_tx_LLR[i] = 0;
            // puncture  4th transmission 
            for(int i=RV1_pos*Z;i<RV2_pos*Z;i++)  First_tx_LLR[i] = 0;
            // Third_tx_LLR copy
            for(int i=0;i<First_tx_LLR.size();i++) Third_tx_LLR[i] = First_tx_LLR[i];
            
            // puncture 3rd  transmission 
            for(int i=RV3_pos*Z;i<PayLoad_H.n;i++) First_tx_LLR[i] = 0;
            
            // Second_tx_LLR copy 
            for(int i=0;i<First_tx_LLR.size();i++) Second_tx_LLR[i] = First_tx_LLR[i];
            // puncture  2nd transmission 
            for(int i=RV2_pos*Z;i<RV3_pos*Z;i++)  First_tx_LLR[i] = 0;

            /* -----------------*/
            bool decode_flag_RV0 = BP_for_Payload(PayLoad_H,iteration_limit,First_tx_LLR,A_CodeWord);
            total_bits += (RV1_pos-RV0_pos)*Z; // total bits
            ...
            /* ------------- Process RV1 ------------- */
            bool decode_flag_RV1 = BP_for_Payload(PayLoad_H,iteration_limit,Second_tx_LLR,A_CodeWord);
            total_bits += (RV3_pos-RV2_pos)*Z;
            if(decode_flag_RV1 == true){
                correct_bits += (PayLoad_H.n - PayLoad_H.m); // info bits
                RV1++;
                continue;
            }
            /* ------------- Process RV2 ------------- */
            bool decode_flag_RV2 = BP_for_Payload(PayLoad_H,iteration_limit,Third_tx_LLR,A_CodeWord);
            total_bits += PayLoad_H.n - RV3_pos*Z;
            if(decode_flag_RV2 == true){
                correct_bits += (PayLoad_H.n - PayLoad_H.m); // info bits
                RV2++;
                continue;
            }
