...
int main(int argc,char* argv[]){
    ...
    string out_file_origin = argv[13];
    int Z = atoi(argv[14]);
    ...
    cout << "total_frame : " << total_frame_count << "\n";
    cout << "##############################" << "\n";
    ...
    ofstream outfile_origin(out_file_origin);
    if (!outfile_origin.is_open()) {
        cout << "Output file: " << out_file_origin  << " cannot be opened." << endl;
		exit(1);
    }
    outfile_origin << "SNR" << "," << "Throught" << "\n";

    ofstream outfile_hybrid(out_file_hybrid);
    if (!outfile_hybrid.is_open()) {
        cout << "Output file: " << out_file_origin  << " cannot be opened." << endl;
		exit(1);
    }
    outfile_hybrid << "SNR" << "," << "Throught" << "\n";
    cout << "fucker \n";
    //------------------------------------------------
    ...
    while(SNR<=SNR_max){
        ...
        for(double frame_count=0;frame_count<total_frame_count;frame_count++){
            ...
            vector<double> Third_LLR_Payload(PayLoad_H.n,0);
            ...
            for(int i=(RV0_pos*Z);i<RV0_pos*Z+(Each_Tx_Znum*Z-(PayLoad_H.n-RV3_pos*Z));i++){
                ...
                Third_LLR_Payload[i] = (Third_LLR_Payload[i]+LLR)/2; // combine LLR
            }
            /*---------------- RV0 ----------------*/
            ...
            if(RV0_flag==true){
                correct_bits_origin += PayLoad_H.n - PayLoad_H.m;
        ...
        outfile_origin << SNR << ", " << throughput_origin << "\n";
        outfile_hybrid << SNR << ", " << throughput_hybrid << "\n";
        cout << "SNR : " << SNR << " | " << "Throughput_origin : " << throughput_origin <<  " |  Throughput_hybrid : " << throughput_hybrid << "\n ";
        cout << "--------------------------------------------\n ";
        SNR += SNR_ratio;
    }
