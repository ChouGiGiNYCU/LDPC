...
int main(int argc,char* argv[]){
    ...
    while(SNR<=SNR_max){
        ...
        for(double frame_count=0;frame_count<total_frame_count;frame_count++){
            ...
            vector<double> Third_tx_LLR(H.n,0);
            ...
            for(int i=RV3_pos*Z;i<PayLoad_H.n;i++){
                ...
                double LLR = 2*receive_value/pow(sigma,2); 
                if(i<(RV2_pos*Z+Each_Tx_Znum*Z)) Third_tx_LLR[i] = (Third_tx_LLR[i]+LLR)/2; // combine LLR
            ...
            /*---------------- RV2 ----------------*/
            for(int i=PayLoad_H.n;i<(PayLoad_H.n+Extra_H.n);i++) Third_tx_LLR[i] = 0;
            // 部分Extra 傳送過來的資訊要放上去
            pair_iter = Superposition_origin.begin();
            for(int i=(PayLoad_H.n+Extra_H.n);i<(PayLoad_H.n+Extra_H.n*2) && pair_iter!=Superposition_origin.end();i++,pair_iter++){
                int origin_vn_pos = pair_iter->second;
                Third_tx_LLR[i] = Third_tx_LLR[origin_vn_pos];
                Third_tx_LLR[origin_vn_pos] = 0; // be punctured
            }
            for(int i=(PayLoad_H.n+Extra_H.n*2);i<(PayLoad_H.n+Extra_H.n*3);i++) Third_tx_LLR[i] = 0; // be punctured
            pair_iter1  = Superpostion_Payload_Extra_NewStructure.begin();
            for(int i=(PayLoad_H.n+Extra_H.n*3);i<(PayLoad_H.n+Extra_H.n*4) && pair_iter1!=Superpostion_Payload_Extra_NewStructure.end();i++,pair_iter1++){
                int Payload_punc_pos = pair_iter1->payload_vn_punc;
                Third_tx_LLR[i] = Third_tx_LLR[Payload_punc_pos];
                Third_tx_LLR[Payload_punc_pos] = 0;
            }
            bool decode_Payload_flag_RV2 = true, decode_Extra_flag_RV2 = true;
            BP_for_CombineH(H,PayLoad_H,Extra_H,iteration_limit,iteration_open,Third_tx_LLR,Payload_CodeWord,decode_Payload_flag_RV2,Extra_CodeWord,decode_Extra_flag_RV2);
            total_bits += Each_Tx_Znum*Z;
            if(decode_Payload_flag_RV1==false && decode_Payload_flag_RV2==true){
                correct_bits += PayLoad_H.n - PayLoad_H.m; // payload info bits + extra info bits
            }
            if(decode_Extra_flag_RV1==false && decode_Extra_flag_RV2==true){
                correct_bits += Extra_H.n - Extra_H.m;
            }
            if(decode_Payload_flag_RV2 && decode_Extra_flag_RV2){
                RV2++;
                continue;
            }
        }
