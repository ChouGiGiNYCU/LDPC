...
int main(int argc,char* argv[]){
    ...
    while(SNR<=SNR_max){
        ...
        for(double frame_count=0;frame_count<total_frame_count;frame_count++){
            ...
            total_bits += (PayLoad_H.n-payload_punc_nums-2*Z); // total bits
            ...
            if(decode_Payload_flag_RV1 && decode_Extra_flag_RV1) RV1++;
