...
int main(int argc,char* argv[]){
    ...
    vector<int> payload_map;
    payload_map = Read_punc_pos(payload_punc_file,payload_punc_nums);
    cout << "Payload Puncture position file read success !!" << endl;
    // define PayLoad_G
    ...
    while(SNR <= SNR_max){
        ...
        while(payload_frame_error < frame_error_lowwer_bound){
            ...
            for(int i=0;i<payload_map.size();i++) receiver_LLR[payload_map[i]] = 0;
            it=0;
