...
H_combine_file = 'PCM_P1008_E96_EnhanceStruc_50percent.txt';
...
for i=1:Total_Payload_punc_bits_num
    Payload_punc_bits_num = 1+size(Already_PuncVNs,2);
    punc_pos_bits = Puncturing_Algorithm_For_Finit_Length(H_combine,Payload_punc_bits_num,Already_PuncVNs); % start idx = 1
    punc_vn = punc_pos_bits(end);
    fprintf("punc_vn : %d\n",punc_vn);
    if punc_vn>H1_c
