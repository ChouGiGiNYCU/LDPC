...
H_combine_file = 'PCM_RCSchemeWeightTree_P1008_E96_NewStrucPartial_50percent.txt';
...
New_puncture_position_bits_outfile = "Punc_Superpostion_new_Payload_Structure_Partial_Combine_50percent.csv";
...
Payload_punc_bits_num = non_Transmit_Extra_VNs_num +  H2_c; % 總共 punc bits 為 Extra + Partial_unTransmit_VNs 
Already_punc_VNs = [];
punc_pos_bits = Puncturing_Algorithm_For_Finit_Length(H1,Payload_punc_bits_num,Already_punc_VNs); % start idx = 1
...
punc_pos_bits_New_Struct2 =  punc_pos_bits(H2_c+1:end);
...
punc_payload_mat_NewStructure1 = zeros([non_Transmit_Extra_VNs_num,H1_c]);
for r=1:non_Transmit_Extra_VNs_num
    punc_vn = punc_pos_bits_New_Struct1(r);
    punc_payload_mat_NewStructure1(r,punc_vn) = 1;
end

punc_payload_mat_NewStructure2 = zeros([non_Transmit_Extra_VNs_num,H1_c]);
for r=1:non_Transmit_Extra_VNs_num
    punc_vn = punc_pos_bits_New_Struct2(r);
...
%%
% H_combine = [  H1     zero1       zero2                     zero2   zero2 
%               zero3     H2        zero4                     zero4   zero4 
%                sup_p   sup_e        I(unsend extra vn)      zero5   zero5
%               puncs1  punc_e      zeor6                       I     zeor6
%               puncs2   zero7      zeor6                       I       I      
%             ] 
zero1 = zeros([H1_r H2_c]);
zero2 = zeros([H1_r non_Transmit_Extra_VNs_num]);
zero3 = zeros([H2_r H1_c]);
zero4 = zeros([H2_r non_Transmit_Extra_VNs_num]);
zero5 = zeros([non_Transmit_Extra_VNs_num non_Transmit_Extra_VNs_num]);
zero6 = zeros([non_Transmit_Extra_VNs_num non_Transmit_Extra_VNs_num]);
zero7 = zeros([non_Transmit_Extra_VNs_num H2_c]);

H_combine = [[H1,zero1,zero2,zero2,zero2];[zero3,H2,zero4,zero4,zero4]];
H_combine = [H_combine;sup_payload_mat,sup_extra_mat,I,zero5,zero5];
H_combine = [H_combine;punc_payload_mat_NewStructure1,sup_extra_mat,zero6,I,zero6];
H_combine = [H_combine;punc_payload_mat_NewStructure2,zero7,zero6,I,I];
%% 輸出檔案
...
T = table(non_Transmit_Extra_VNs.',punc_pos_bits_New_Struct1.',punc_pos_bits_New_Struct2.','VariableNames', {'Extra bits','Payloadb_NonPunc(b)', 'Payloadc_Punc(c)'});  % 建立 table
