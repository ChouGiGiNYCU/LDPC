...
int main(int argc,char* argv[]){
    double frame_error_lowwer_bound = 100;
    if(argc < 2){
    ...
    string Superposition_origin_file = argv[13]; // 原本的方法做疊加，Extra/Payload都做punc
    string Table_ExtraTransmit_PayloadPunc_file = argv[14]; // 部分extra 傳送，部分payload不傳
    string Superpostion_Payload_Extra_NewStructure_pos_file = argv[15]; // 新的結構 一個被punc過的Extra 會跟 兩個payload bits做結合
    int Z = atoi(argv[16]);
    string Frozen_bits_file = argv[17];
    string Non_Frozen_bits_file = argv[18];
    cout << "##############################" << "\n";
    ...
    cout << "Frozen_bits_file : " << Non_Frozen_bits_file << "\n";
    ...
    boost::dynamic_bitset<> extra_Encode(Extra_G[0].size());
