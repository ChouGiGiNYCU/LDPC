...
H_combine_file = 'PCM_P1008_E96_kSR_EnhanceStruc_50percent.txt';
...
for r=1:non_Transmit_Extra_VNs_num
    punc_vn = non_Transmit_Extra_VNs(r);
    Already_PuncVNs = union(Already_PuncVNs,H1_c+punc_vn);
end

Total_Payload_punc_bits_num = H2_c * 2 ; % 總共 punc bits 為 Extra(部分Superposition、Extra傳送) + New_Structure2
punc_pos_bits_origin = [];
payload_punc_unsend  = []; 
punc_pos_bits_New_Struct2 = [];
Can_Punc_VNs = 1:H1_c;
for i=1:Total_Payload_punc_bits_num
    Payload_punc_bits_num = 1;
    punc_pos_bits = Rate_Compatible_Punctured_With_Short_Block_Lengths(H_combine,Payload_punc_bits_num,Already_PuncVNs,Can_Punc_VNs); % start idx = 1
    fprintf("punc_vn : %d\n",punc_vn);
    punc_vn = punc_pos_bits;
    Can_Punc_VNs = setdiff(Can_Punc_VNs,punc_vn);
    if punc_vn>H1_c
        fprintf("Error Punc_VN is out of range bound !! ----------------------\n");
    else
        Already_PuncVNs = union(Already_PuncVNs,punc_vn);
    end
    if i<=non_Transmit_Extra_VNs_num
        punc_pos_bits_origin = [punc_pos_bits_origin,punc_vn]; % Superposition
        H_combine(H1_r+H2_r+i,punc_vn) = 1;
    elseif i>non_Transmit_Extra_VNs_num && i<=H2_c
        payload_punc_unsend = [payload_punc_unsend,punc_vn];  % Partial Transmit Extra(payload不傳送的)
    elseif i>H2_c
        punc_pos_bits_New_Struct2 = [punc_pos_bits_New_Struct2,punc_vn]; % 被 punc
        H_combine(H1_r+H2_r+non_Transmit_Extra_VNs_num+i,punc_vn) = 1; 
    end
    Non_select_VNs = setdiff(Non_select_VNs,punc_vn);
end
...
% punc_pos_bits_New_Struct1 =  punc_pos_bits(H2_c+1:H2_c*2); % 沒有被punc掉(跟Extra 做疊加)
...
for r=1:H2_c
    ...
    while true
        ...
        if any(ismember(Nei_CN_VNs_set,choose_VN))
            ...
            H_combine(H1_r+H2_r+non_Transmit_Extra_VNs_num+r,choose_VN) = 1;
