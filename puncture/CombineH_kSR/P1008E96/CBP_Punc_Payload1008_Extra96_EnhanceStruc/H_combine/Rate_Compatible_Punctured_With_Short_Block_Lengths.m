function Punc_VNs = Rate_Compatible_Punctured_With_Short_Block_Lengths(H,punc_bits_num,Already_Punc_VNs,Can_Punc_VNs)
    %% init
    Punc_VNs = [];
    [rows,cols] = size(H);
    VN2CN_pos = {};
    ...
    %% find punc node
    for i=2:k
        if punc_bits_num==length(Punc_VNs)
            break;
        end
        ...
        if Vns_num < diff
            ...
            while length(Punc_VNs)<punc_bits_num
                ...
                Candidate_VNs = [];
                for VN=G{i}
                    if intersect(VN,Can_Punc_VNs)==0
                        continue;
                    end
