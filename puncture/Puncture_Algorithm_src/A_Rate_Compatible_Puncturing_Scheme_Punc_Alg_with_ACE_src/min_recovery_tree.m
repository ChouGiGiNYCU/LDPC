function U_CN = min_recovery_tree(H,CN2VN_pos,VN2CN_pos,Punc_VNs,CNs)
    ...
    for punc_vn=copy_Punc_VNs
        min_NeiVNs = 1e9;
        % 找出 1-SR 最小能被 recover 的 unpunc 數量 
        for Nei_CN=VN2CN_pos{punc_vn}
            Nei_CN_VNs = setdiff(CN2VN_pos{Nei_CN},punc_vn); % 把 Punc_VN 排除掉
        ...
        if min_NeiVNs~=1e9
            ...
            Already_recover_PuncVNs = [Already_recover_PuncVNs,punc_vn]; 
        end
    end
    fprintf("Already process all SR node\n");
                ...
                for Nei_CN_VN=Nei_CN_VNs 
                    ...
                    if any(ismember(Punc_VNs,Nei_CN_VN)) % 此Nei_CN_VN 為 Punc_VN
                        if min_NeiVNs_to_PuncVNs_num(Nei_CN_VN)~=0 % 此Nei_CN_VN 已經找到最小的recovery tree了 
                            Nei_CN_CanRecovery_PuncVN_num = Nei_CN_CanRecovery_PuncVN_num + min_NeiVNs_to_PuncVNs_num(Nei_CN_VN)*k_SR;
                        else
