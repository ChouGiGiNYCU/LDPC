...
H_combine_file = 'PCM_P1008_E96_PartialExtraTransmit_1SR_22_Random.txt';
puncture_position_bits_file = "Table_Superposition_Extra_Payload_22_Random.csv"; % 對應payload 、 extra puncture位置(原本的方法)
Transmit_Extra_VNs_table_file     = "Table_ExtraTransmitVNs_to_PuncPosPayload_22_Random.csv"; % 傳送的extra bits 位置 、不傳送的payload bits
VN2Girth_map_file = "VN2Girthmap_GirthRandom.csv";
...
Extra_Transmit_number = 22;
% 把Extra VN 分成 Transmit and Superposition 
Each_VNs_Girth_num = readtable(Each_VNs_Girth_num_file); % read Each_VNs_Girth_num_file 
vn = Each_VNs_Girth_num.VN; 
girth_num = Each_VNs_Girth_num.Girth;
%%  找出 girth 較多的 VNs
% [~, idx] = sort(girth_num, 'descend'); % 根據 girth_node 的值做遞減排序
[~, idx] = sort(girth_num, 'ascend'); % 根據 girth_node 的值做遞增排序
vn_sorted = vn(idx); % 排序後的 vn 與 girth_node
girth_sorted = girth_num(idx); 

%% choose Extra Transmit method(1) - 利用girth method 來選Extra Transmit
% 要傳送的部分 Extra VNs
T = table(vn_sorted(1:Extra_Transmit_number),girth_sorted(1:Extra_Transmit_number),'VariableNames', {'VN', 'pass_Girth_num'});  % 建立 table
writetable(T, VN2Girth_map_file);  % write Transmit VN-girth map to excel 
Transmit_Extra_VNs = vn_sorted(1:Extra_Transmit_number).' + 1; % start idx =1
remaining_Extra_VN = setdiff(1:H2_c, Transmit_Extra_VNs);  % 找出剩下的部分
%% choose Extra Transmit method(2) - random choose Extra transmit VNs 
Transmit_Extra_VNs = randperm(H2_c,Extra_Transmit_number); % random choose(idx) start idx = 1
remaining_Extra_VN = setdiff(1:H2_c, Transmit_Extra_VNs);  % 找出剩下的部分
T = table(Each_VNs_Girth_num.VN(Transmit_Extra_VNs),Each_VNs_Girth_num.Girth(Transmit_Extra_VNs),'VariableNames', {'VN', 'pass_Girth_num'});  % 建立 table
writetable(T, VN2Girth_map_file);  % write Transmit VN-girth map to excel 
%% choose Extra Transmit method(3) - Extra transmit non 1-SR node 
