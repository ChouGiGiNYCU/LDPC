...
close all;
%%
H1_infile = 'C:\Users\USER\Desktop\LDPC\PCM\PEGReg504x1008.txt'; % payload data
H2_infile = 'C:\Users\USER\Desktop\LDPC\PCM\H_96_48.txt'; % extra data
H_combine_outfile = 'PCM_P1008_E96_Structure.txt';
puncture_position_bits_outfile = "Punc_Superpostion_origin_Payload.txt";
New_puncture_position_bits_outfile = "Punc_Superpostion_new_Payload.csv";

H1 = readHFromFileByLine(H1_infile);
H2 = readHFromFileByLine(H2_infile);
[H1_r,H1_c] = size(H1);
[H2_r,H2_c] = size(H2);
...
%% punc 1-SR 
punc_pos_bits = maximize_oneSR_method(H1,H2_c*2); % idx = 1
% 把找到的punc bits 分成 Payload+Extra 、 New_Struct(e^b) 、 New_Struct(e^b^c)
punc_pos_bits_origin      =  punc_pos_bits(1:H2_c); % 疊加的部分 Superposition
punc_pos_bits_New_Struct2 =  punc_pos_bits(H2_c+1:H2_c*2);
...
writePCM(H_combine,H_combine_outfile);
...
fileID = fopen(puncture_position_bits_outfile, 'w');
