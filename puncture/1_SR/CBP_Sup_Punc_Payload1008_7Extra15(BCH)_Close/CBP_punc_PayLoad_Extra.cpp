...
int main(int argc,char* argv[]){
    ...
    int Extra_PCM_number = atoi(argv[14]);
    cout << "##############################" << "\n";
    ...
    cout << "Extra_PCM_number : " << Extra_PCM_number << "\n";
    cout << "##############################" << "\n";
    // define H_combine
    ...
    vector<int> punc_map;
    punc_map = Read_punc_pos(puncture_pos_file,punc_pos,Extra_H.n*Extra_PCM_number);
    cout << "Puncture position file read success | size : " << punc_map.size() <<  "\n";
    cout << "##############################" << "\n";
    // define PayLoad_G
    ...
    if(Extra_Flag){
        ...
        if(punc_map.size()==(Extra_H.n*Extra_PCM_number)) cout << "Punc size euqal to Extra_PCM number!!\n";
        else throw invalid_argument("-- Error : Punc_map size != Extra_PCM_number * Extra_H.colsize !!\n");
        cout << "##############################" << "\n";
    }
    // create payload out csv
    ofstream PayLoad_outfile(PayLoad_out_file);
    if (!PayLoad_outfile.is_open()) {
        cout << "Output file: " << PayLoad_out_file  << " cannot be opened.\n";
		exit(1);
    }
    ...
    double SNR = SNR_min;
    // ((Payload info) + length(Extra info)*Extra_PCM_number)/ length(Payload CodeWord) 
    double code_rate = (double)(PayLoad_H.n-PayLoad_H.m+Extra_Origin_G.size()*Extra_PCM_number)/(double)PayLoad_H.n;
    ...
    // Assuming H.n and H.m are defined somewhere
    ...
    vector<boost::dynamic_bitset<>> extra_guess(Extra_PCM_number,boost::dynamic_bitset<>(Extra_H.n));
    ...
    while(SNR <= SNR_max){
        ...
        int it;
        fill(payload_bit_error_count.begin(), payload_bit_error_count.end(), 0.0); // reset to zero
        fill(extra_bit_error_count.begin(), extra_bit_error_count.end(), 0.0);     // reset to zero
        clk_start = clock();
        while(payload_frame_error < frame_error_lowwer_bound){
            // 處理 Payload CodeWord 是zero 還是Encode
            if(PayLoad_Flag){
                for(int k=0;k<Payload_Origin_G.size();k++){
                    payload_info[k] = random_generation();
                }
            ...
            // 假設鮮每一組 Extra CodeWord都一樣
            if(Extra_Flag){
                for(int k=0;k<Extra_Origin_G.size();k++){
                    extra_info[k] = random_generation();
                }
            ...
            for(int i=0;i<PayLoad_H.n;i++){
                if(PayLoad_Flag) transmit_codeword[i] = payload_Encode[i];
                else transmit_codeword[i] = 0;
            }
            for(int PCM=0;PCM<Extra_PCM_number;PCM++){
                int base_map_pos = PCM * Extra_H.n; 
                for(int i=0;i<Extra_H.n;i++){
                    int xor_pos = punc_map[base_map_pos+i];
            ...
            // setting puncture Extra bits
            for(int i=PayLoad_H.n;i<(PayLoad_H.n+Extra_PCM_number*Extra_H.n);i++){
                receiver_LLR[i] = 0;
            }
            // swap LLR value with superposition_pos & punc_pos
            for(int i=(PayLoad_H.n+Extra_PCM_number*Extra_H.n);i<H.n;i++){
                int idx = i-(PayLoad_H.n+Extra_PCM_number*Extra_H.n);
            ...
            while(it<iteration_limit && (payload_error_syndrome || extra_error_syndrome)){
                ...
                for(int VN=0;VN<H.n;VN++){
                    // 不把訊息傳到Extra 那邊
                    if(VN>=PayLoad_H.n && VN<(Extra_H.n*Extra_PCM_number+PayLoad_H.n) && (it<iteration_open && payload_correct_flag==false)) continue;
                ...
                for(int PCM=1;PCM<=Extra_PCM_number;PCM++){
                    int start_VN = PayLoad_H.n + Extra_H.n * (PCM-1);
                    int end_VN   =  PayLoad_H.n + Extra_H.n * PCM;
                    for(int VN=start_VN;VN<end_VN;VN++){
                        extra_guess[PCM-1][VN-start_VN] = totalLLR[VN]>0?0:1;
                    }
                }
                ...
                int Each_extra_error_bit = 0;
                for(int PCM=0;PCM<Extra_PCM_number;PCM++){
                    auto extra_check_result =  extra_guess[PCM] ^ extra_Encode;
                    Each_extra_error_bit = extra_check_result.count();
                    if(Each_extra_error_bit>0) extra_bit_error_flag = true;
                    extra_bit_error_count[it] += static_cast<double>(Each_extra_error_bit);
                    extra_error_bit += Each_extra_error_bit;
                }
        ...
        printf("PayLoad - SNR : %.2f  | FER : %.5e  | BER : %.5e(%d) | Time : %.3f | Total frame : %.0f\n",SNR,payload_frame_error/frame_count,payload_bit_error_count[iteration_limit-1]/(static_cast<double>(PayLoad_H.n)*frame_count),it,clk_duration,frame_count);
        printf("Extra   - SNR : %.2f  | FER : %.5e  | BER : %.5e(%d) | Time : %.3f | Error frame : %.0f | BER_num : %.0f\n",SNR,extra_frame_error/frame_count,extra_bit_error_count[iteration_limit-1]/(static_cast<double>(Extra_H.n*frame_count*Extra_PCM_number)),it,clk_duration,extra_frame_error,extra_bit_error_count[iteration_limit-1]);
        
        // Write PayLoad Performance to csv
        PayLoad_outfile << SNR << ", " << payload_frame_error/frame_count << ", ";
        for(int i=0;i<iteration_limit;i++){
            PayLoad_outfile << payload_bit_error_count[i]/(static_cast<double>(PayLoad_H.n)*frame_count) << ", ";
        }
        PayLoad_outfile <<endl;
        ...
        for(int i=0;i<iteration_limit;i++){
            Extra_outfile << extra_bit_error_count[i]/(static_cast<double>(Extra_H.n)*frame_count*Extra_PCM_number) << ", ";
        }
        Extra_outfile <<endl;
